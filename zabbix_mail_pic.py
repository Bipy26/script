...
graph_url = 'http://192.168.1.1/chart.php'  # 定义图表的url
# api_url ="http://192.168.1.1/api_jsonrpc.php"    #定义api的url
# header = {"Content-Type":"application/json" }     #定义api的headers
loginurl = "http://192.168.1.1/index.php"  # 定义登录的url
host = '192.168.1.1'
to_email = sys.argv[1]  # 传入的第一个参数为收件人邮箱
subject = sys.argv[2]  # 传入的第二个参数为邮件主题
subject = subject.decode('utf-8')
smtp_host = 'xxx.com'  # 定义smtp主机地址
from_email = 'xxx@xxx.com'  # 定义发件人地址
mail_pass = 'xxx'  # 发件人邮箱校验码


def get_itemid():
    # 获取报警的itemid
    if re.search(r'ITEM ID:(\d+)', sys.argv[3]) == None:
        itemid = 0
        return itemid
    else:
